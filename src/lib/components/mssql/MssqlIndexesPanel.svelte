...
<div class="flex h-full flex-col gap-3">
	<div class="flex items-center gap-2 p-2">
		<span class="text-muted-foreground text-xs">Indexes</span>
		...
		{:else}
			<table class="w-full text-sm">
				...
				<tbody>
					...
					{#if data.length === 0}
						<tr>
							<td class="text-muted-foreground p-4" colspan="5">No indexes</td>
						</tr>
					{/if}
				</tbody>
			</table>
	...
	<div class="flex items-center gap-2 p-2">
		<button
			...
			onclick={prevPage}
			disabled={loading || page <= 0}>Prev</button
		>
		<span class="text-xs">Page {page + 1}</span>
		<button
			class="enabled:hover:bg-accent rounded-sm border px-3 py-1 text-sm disabled:opacity-50"
			onclick={nextPage}
			disabled={loading || page + 1 >= totalPages}>Next</button
		>
