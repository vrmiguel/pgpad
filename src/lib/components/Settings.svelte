...
<div class="flex h-full flex-col gap-4 p-4">
    <div class="flex items-center justify-between">
        ...
        <div class="sticky top-0 z-10 flex items-center gap-3 flex-wrap">
            <label class="flex items-center gap-2 text-sm">
                <span>Scope</span>
                <select class="rounded border px-2 py-1 w-28"
                    bind:value={scope}
    ...
    {:else}
        <div class="rounded-lg border p-5">
            ...
            <p class="text-sm text-muted-foreground">Oracle‑specific settings have been moved to the connection dialog (Edit → Oracle Settings). Reconnect and formatting options are now configured per connection.</p>
        </div>
    {/if}
</div>
