...
impl ParsingContext {
    ...
    pub fn did_change(&mut self, new_text: String) -> crate::Result<()> {
        ...
        if let Some(tree) = self.parser.parse(&self.query, self.current_tree.as_ref()) {
            ...
            if tree_end < self.query.len() {
                println!(
                    "DEBUG: WARNING - Tree only parsed {} of {} bytes. Remaining text: '{}'",
                    tree_end,
                    self.query.len(),
                    &self.query[tree_end..]
                );
                ...
                self.check_for_errors(&mut cursor, &mut has_errors);
                if has_errors {
                    println!("DEBUG: Parse tree contains ERROR nodes");
                }
            }
            ...
            println!("DEBUG: Failed to parse new text");
    ...
    pub fn suggestion_for_position(&mut self, position: Point) -> Result<SuggestionType, String> {
        ...
        let need_reparse = match &self.current_tree {
            Some(tree) => {
                ...
                println!(
                    "DEBUG: Checking if reparse needed - tree: {} bytes, query: {} bytes",
                    tree_text_len, query_len
                );
                tree_text_len != query_len // Tree is stale if lengths don't match
            }
            None => {
                println!("DEBUG: No existing tree, need to parse");
                true // No tree at all
            }
        };
        ...
        if need_reparse {
            if let Some(tree) = self.parser.parse(&self.query, self.current_tree.as_ref()) {
                let tree_end = tree.root_node().end_byte();
                println!(
                    "DEBUG: Reparsed tree (root ends at byte {}, query len: {})",
                    tree_end,
                    self.query.len()
                );
                ...
                // Check if the tree parsed the entire query
                if tree_end < self.query.len() {
                    println!("DEBUG: WARNING - Reparsed tree only covers {} of {} bytes. Remaining: '{}'", 
                             tree_end, self.query.len(), &self.query[tree_end..]);
                    ...
                    self.check_for_errors(&mut cursor, &mut has_errors);
                    if has_errors {
                        println!("DEBUG: Reparsed tree contains ERROR nodes");
                    }
                }

                self.current_tree = Some(tree.clone());
            } else {
                println!("DEBUG: Parsing failed for query: '{}'", self.query);
    ...
    /// Test simple queries to debug grammar issues
    fn test_simple_queries(&mut self) {
        ...
        for query in test_queries {
            if let Some(tree) = self.parser.parse(query, None) {
                let tree_end = tree.root_node().end_byte();
                println!(
                    "DEBUG: Test query '{}' (len: {}) -> tree ends at byte {}",
                    query,
                    query.len(),
                    tree_end
                );
                if tree_end < query.len() {
                    println!("DEBUG:   Remaining unparsed: '{}'", &query[tree_end..]);
                }
            } else {
                println!("DEBUG: Test query '{}' failed to parse", query);
            }
        }
    }
    ...
    fn check_for_errors(&self, cursor: &mut tree_sitter::TreeCursor, has_errors: &mut bool) {
        if cursor.node().kind() == "ERROR" {
            *has_errors = true;
            println!(
                "DEBUG: Found ERROR node at byte {}-{}: '{}'",
                cursor.node().start_byte(),
                cursor.node().end_byte(),
                cursor
                    .node()
                    .utf8_text(self.query.as_bytes())
                    .unwrap_or("???")
            );
        }
        ...
        if cursor.goto_first_child() {
            loop {
                self.check_for_errors(cursor, has_errors);
                if !cursor.goto_next_sibling() {
                    break;
                }
            }
    ...
    fn extract_table_references(&mut self, tree: &Tree) {
        // Validate that the tree is compatible with current query
        let tree_text_len = tree.root_node().end_byte();
        let query_len = self.query.len();
        ...
        self.print_tree_debug(tree.root_node(), 0);

        // Check for ERROR nodes that might be stopping parsing
        let mut cursor = tree.walk();
        let mut has_errors = false;
        self.check_for_errors(&mut cursor, &mut has_errors);
        if has_errors {
            println!("DEBUG: Parse tree contains ERROR nodes - this explains why parsing stopped");
        } else {
            println!("DEBUG: No ERROR nodes found, parser just stopped for unknown reason");
        }
            ...
            for capture in query_match.captures {
                ...
                if capture_name == "table.name" {
                    ...
                    if !cleaned_name.is_empty() && !self.is_sql_keyword(cleaned_name) {
                        println!(
                            "DEBUG: Found table name: {} (cleaned: {})",
                            table_name, cleaned_name
                        );
                        self.scope
                            .available_tables
                            .insert(cleaned_name.to_string(), cleaned_name.to_string());
                    } else if self.is_sql_keyword(cleaned_name) {
                        println!("DEBUG: Skipping SQL keyword: {}", cleaned_name);
                    }
                }
            }
        }
    ...
    fn print_tree_debug(&self, node: tree_sitter::Node, depth: usize) {
        let indent = "  ".repeat(depth);

        // Safely get node text with bounds checking
        let node_text = match node.utf8_text(self.query.as_bytes()) {
            Ok(text) => text.chars().take(50).collect::<String>(),
            Err(_) => {
                // Fallback: manually extract text with bounds checking
                let start = node.start_byte();
                let end = node.end_byte();
                if start <= self.query.len() && end <= self.query.len() && start <= end {
                    self.query[start..end].chars().take(50).collect::<String>()
                } else {
                    format!("<invalid_range:{}-{}>", start, end)
                }
            }
        };
        ...
        if depth < 3 {
            ...
            if cursor.goto_first_child() {
                loop {
                    self.print_tree_debug(cursor.node(), depth + 1);
                    if !cursor.goto_next_sibling() {
                        break;
                    }
                }
...
mod tests {
    ...
    fn test_sql_parsing_suggestion_types() {
            ...
            test_queries.into_iter().zip(expected_types.into_iter())
        {
            ...
            println!(
                "✓ Query '{}' correctly suggests: {:?}",
                query,
                get_suggestion_type_name(&actual)
            );
        }

        println!("All SQL parsing suggestion type tests passed!");
    }
