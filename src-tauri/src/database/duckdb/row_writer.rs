        ...
        for i in 0..self.column_decltypes.len() {
            ...
            match row.get_ref(i)? {
                ...
                ValueRef::Time64(unit, v) => {
                    ...
                    if sub_us == 0 {
                        self.write_json_string(&format!("{:02}:{:02}:{:02}", h, m, s));
                    } else {
                ...
                ValueRef::Timestamp(unit, v) => {
                    ...
                    if let Some(dt) = dt {
                        self.write_json_string(&dt.to_string());
                    } else {
                        self.write_json_string(&format!("{}", v));
                    }
                }
                ValueRef::Interval {
                    months,
                    ...
                    let time_part = if sub_us == 0 {
                        format!("{:02}:{:02}:{:02}", h, m, s)
                    } else {
                        format!("{:02}:{:02}:{:02}.{:06}", h, m, s, sub_us)
                    };
                ...
                ValueRef::Text(value) => {
                    let ty_is_json = self
                        ...
                        .and_then(|o| o.as_ref())
                        .map(|s| {
                            s.eq_ignore_ascii_case("json") || s.to_lowercase().contains("json")
                        })
                        .unwrap_or(false);
                    let is_json = if ty_is_json {
                        true
    ...
    fn write_duck_value_json(&mut self, v: &Value) {
        match v {
            ...
            Value::Timestamp(unit, v) => {
                ...
                let sub_us = micros.rem_euclid(1_000_000);
                let dt =
                    chrono::DateTime::<chrono::Utc>::from_timestamp(secs, (sub_us as u32) * 1000);
                if let Some(dt) = dt {
                    self.write_json_string(&dt.to_string());
                } else {
                    self.write_json_string(&format!("{}", v));
            ...
            Value::Time64(unit, v) => {
                ...
                if sub_us == 0 {
                    ...
                    self.write_json_string(&format!("{:02}:{:02}:{:02}.{:06}", h, m, s, sub_us));
                }
            }
            Value::Interval {
                months,
            ...
            Value::Union(inner) => {
                self.write_duck_value_json(inner);
            }
        }
    }
}
