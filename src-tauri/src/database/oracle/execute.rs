...
use oracle::sql_type::{Timestamp, ToSql};
...
pub fn execute_query_with_params(
    ...
    let named: Vec<(String, Box<dyn ToSql>)> = params
        .iter()
        .map(|(k, v)| (k.clone(), to_sql_box(v)))
        .collect();
    let named_refs: Vec<(&str, &dyn ToSql)> = named
        .iter()
        ...
        match client.query_named(&stmt.statement, &named_refs[..]) {
            Ok(rows) => {
                ...
                let mut writer = match settings {
                    Some(s) => {
                        RowWriter::with_settings(cols.to_vec(), Some(s)).with_sender(sender.clone())
                    }
                    None => RowWriter::new(cols.to_vec()).with_sender(sender.clone()),
                };
                for (idx, row_res) in rows.enumerate() {
                    let row =
                        row_res.map_err(|e| Error::Any(anyhow::anyhow!("Row error: {}", e)))?;
                    writer
                        .add_row(&row, idx)
                        .map_err(|e| Error::Any(anyhow::anyhow!("Row add error: {}", e)))?;
...
fn execute_dml_returning(
    ...
    let col_types = if !owner.is_empty() && !table_name.is_empty() {
        fetch_column_types(client, &owner, &table_name, &ret_cols)
    } else {
    ...
    for name in &bind_names_owned {
        ...
        if let Some(v) = params.get(&key) {
            stmt.bind(name.as_str(), &*to_sql_box(v))
                .map_err(|e| Error::Any(anyhow::anyhow!(e)))?;
        } else if let Some(idx) = out_names.iter().position(|n| n.eq_ignore_ascii_case(name)) {
            let col = &ret_cols[idx];
            if let Some(ot) = col_types.get(&col.to_uppercase()) {
                stmt.bind(name.as_str(), ot)
                    .map_err(|e| Error::Any(anyhow::anyhow!(e)))?;
            } else {
                stmt.bind(name.as_str(), &oracle::sql_type::OracleType::Varchar2(4000))
                    .map_err(|e| Error::Any(anyhow::anyhow!(e)))?;
            }
        } else {
            stmt.bind(name.as_str(), &oracle::sql_type::OracleType::Varchar2(4000))
    ...
    match stmt.execute(&[]) {
        Ok(()) => {
            ...
            for name in &out_names {
                let v: String = stmt
                    .bind_value(name.as_str())
                    .map_err(|e| Error::Any(anyhow::anyhow!(e)))?;
                values.push(v);
            }
            let page_json = json!([values]);
            let page =
                JsonRawValue::from_string(serde_json::to_string(&page_json).unwrap()).unwrap();
            sender.send(QueryExecEvent::Page {
                page_amount: 1,
                page,
            })?;
...
fn parse_dml_table(sql: &str, client: &oracle::Connection) -> (String, String) {
    ...
    if owner.is_empty() {
        if let Ok(mut r) = client.query("SELECT USER FROM DUAL", &[]) {
            if let Some(Ok(row)) = r.next() {
                owner = row
                    .get::<usize, String>(0)
                    .unwrap_or_default()
                    .to_uppercase();
...
fn fetch_column_types(
    client: &oracle::Connection,
    owner: &str,
    table: &str,
    cols: &[String],
) -> std::collections::HashMap<String, oracle::sql_type::OracleType> {
    ...
    let subset: std::collections::HashMap<String, oracle::sql_type::OracleType> = cols
        .iter()
        .map(|c| c.to_uppercase())
        .filter_map(|n| map.get(&n).map(|ot| (n.clone(), ot.clone())))
        .collect();
    subset
}
        ...
        Ok(()) => {
            ...
            for name in &out_names {
                let v: String = stmt
                    .bind_value(name.as_str())
                    .map_err(|e| Error::Any(anyhow::anyhow!(e)))?;
                if let Some((_, _, oracle::sql_type::OracleType::Number(_, s))) =
                    specs.iter().find(|(n, _, _)| n.eq_ignore_ascii_case(name))
                {
                    values.push(crate::database::oracle::numeric::pad_number_scale(
                        &v,
                        (*s).try_into().unwrap_or(0),
                    ));
                } else {
                    values.push(v);
                }
...
fn normalize_binds_cached(
    ...
    if let Ok(map) = cache.lock() {
        if let Some((norm, names)) = map.get(sql) {
            let boxed = names
                .iter()
                .map(|n| to_sql_box(params.get(n).unwrap_or(&serde_json::Value::Null)))
                .collect();
    ...
    if let Ok(mut map) = cache.lock() {
        map.insert(sql.to_string(), (norm.clone(), names));
    }
    (norm, boxed)
}
...
fn normalize_binds(
    ...
    let boxed: Vec<Box<dyn ToSql>> = names
        .iter()
        .map(|n| to_sql_box(params.get(n).unwrap_or(&serde_json::Value::Null)))
        .collect();
    (out, boxed)
    ...
    match v {
        ...
        serde_json::Value::Number(num) => {
            if let Some(i) = num.as_i64() {
                ...
                Box::new(num.as_f64().unwrap_or(0.0)) as Box<dyn ToSql>
            }
        }
        serde_json::Value::String(s) => {
            if let Some(ts) = parse_timestamp(s) {
                Box::new(ts) as Box<dyn ToSql>
            } else {
                Box::new(s.clone()) as Box<dyn ToSql>
            }
        }
...
fn execute_query_with_results(
    ...
    match client.query(query, &[]) {
        Ok(rows) => {
            ...
            log::info!(
                "Oracle query completed: {} rows in {}ms",
                total_rows,
                duration
            );
            sender.send(QueryExecEvent::Finished {
...
fn execute_explain_plan(
    ...
    match client.execute(stmt, &[]) {
        Ok(_) => {
            let fmt = settings
                .and_then(|s| s.xplan_format.clone())
                .unwrap_or_else(|| {
                    std::env::var("ORACLE_XPLAN_FORMAT").unwrap_or_else(|_| "TYPICAL".to_string())
                });
            let mode = settings
                .and_then(|s| s.xplan_mode.clone())
                .unwrap_or_else(|| String::from("display"));
            ...
            match client.query(&q, &[]) {
                Ok(rows) => {
                    ...
                    sender.send(QueryExecEvent::TypesResolved { columns })?;
                    let page =
                        JsonRawValue::from_string(serde_json::to_string(&page_json).unwrap())
                            .unwrap();
                    sender.send(QueryExecEvent::Page {
                        page_amount: page_json.as_array().map(|a| a.len()).unwrap_or(0),
                        page,
                    })?;
                    sender.send(QueryExecEvent::Finished {
                        elapsed_ms: started_at.elapsed().as_millis() as u64,
...
fn format_dbms_xplan(lines: &[String]) -> (Vec<String>, serde_json::Value) {
    ...
    let arr = serde_json::Value::Array(
        lines
            .iter()
            .map(|s| serde_json::Value::Array(vec![serde_json::Value::String(s.clone())]))
            .collect(),
    );
    (headers, arr)
}
...
fn execute_describe(
    ...
    let s = stmt.trim();
    let name = if s.to_uppercase().starts_with("DESCRIBE ") {
        &s[9..]
    } else {
        &s[5..]
    };
    ...
    // Check object type
    let obj_type = match client.query(
    ...
    if obj_type == "PACKAGE" || obj_type == "PACKAGE BODY" {
        ...
        match client.query(sql, &[&owner, &obj_name]) {
            Ok(rows) => {
                ...
                let batch_size = env::var("PGPAD_BATCH_SIZE")
                    .ok()
                    .and_then(|v| v.parse::<usize>().ok())
                    .filter(|&n| n > 0)
                    .unwrap_or(50);
...
pub fn detect_out_args(client: &oracle::Connection, sql: &str) -> Vec<String> {
    ...
    let up = sql.to_uppercase();
    let target = if up.starts_with("CALL ") {
        sql.trim()[5..].trim().to_string()
    } else {
    ...
    if !name.is_empty() {
        ...
        match parts.len() {
            3 => {
                owner = parts[0].trim().to_uppercase();
                package = parts[1].trim().to_uppercase();
                object = parts[2].trim().to_uppercase();
            }
            2 => {
                package = parts[0].trim().to_uppercase();
                object = parts[1].trim().to_uppercase();
            }
            1 => {
                object = parts[0].trim().to_uppercase();
            }
            _ => {}
        }
        if owner.is_empty() {
        ...
        let binds: Vec<&dyn ToSql> = if package.is_empty() {
            vec![&owner, &object]
        } else {
            vec![&owner, &object, &package]
        };
        if let Ok(mut rows) = client.query(sql, &binds[..]) {
            let mut names = Vec::new();
            while let Some(Ok(row)) = rows.next() {
                if let Ok(n) = row.get::<usize, String>(0) {
                    names.push(n);
                }
            }
            return names;
...
pub fn detect_out_specs(client: &oracle::Connection, sql: &str) -> OutSpecs {
    ...
    if !name.is_empty() {
        ...
        if let Ok(mut rows) = client.query(sql, &binds[..]) {
            ...
            if let Ok(mut m) = cache.lock() {
                m.insert(sql.to_string(), specs.clone());
            }
            return specs;
        }
    }
...
fn map_oracle_type(dtype: &str, len: i32, prec: i16, scale: i16) -> oracle::sql_type::OracleType {
    ...
    if t.contains("VARCHAR") || t == "CHAR" {
        oracle::sql_type::OracleType::Varchar2(std::cmp::max(1u32, len.try_into().unwrap_or(1)))
    } else if t == "NUMBER" {
        let p = if prec <= 0 { 38 } else { prec };
        oracle::sql_type::OracleType::Number(p.try_into().unwrap(), scale.try_into().unwrap())
    } else if t == "DATE" {
        oracle::sql_type::OracleType::Varchar2(64)
    } else if t.contains("TIMESTAMP") {
        oracle::sql_type::OracleType::Varchar2(128)
    } else if t == "RAW" {
        oracle::sql_type::OracleType::Raw(std::cmp::max(1u32, len.try_into().unwrap_or(1)))
    } else if t == "BLOB" {
        oracle::sql_type::OracleType::BLOB
    } else if t == "CLOB" {
        oracle::sql_type::OracleType::CLOB
    } else {
        oracle::sql_type::OracleType::Varchar2(std::cmp::max(1u32, len.try_into().unwrap_or(1)))
    }
}
