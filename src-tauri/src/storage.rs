...
impl Storage {
    ...
    pub fn get_query_history(&self, connection_id: &str, limit: Option<i64>) -> Result<Vec<QueryHistoryEntry>> {
        ...
        let rows = stmt.query_map((connection_id, limit), |row| {
    ...
    pub fn get_setting(&self, key: &str) -> Result<Option<String>> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare("SELECT value FROM app_settings WHERE key = ?1").context("Failed to prepare settings statement")?;
        let mut rows = stmt.query_map([key], |row| row.get::<_, String>(0)).context("Failed to query settings")?;
    ...
    pub fn delete_saved_query(&self, id: i64) -> Result<()> {
        let conn = self.conn.lock().unwrap();
        conn.execute("DELETE FROM saved_queries WHERE id = ?1", [id]).context("Failed to delete saved query")?;
        Ok(())
    }
}
