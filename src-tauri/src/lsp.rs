...
impl LanguageServer {
    ...
    fn handle_completion(&self, request: Request) -> Result<(), Error> {
        ...
        // Add SQL keywords (context-aware)
        for keyword in SQL_KEYWORDS {
            // Apply context-based filtering similar to validFor
            if self.should_include_keyword(&keyword.keyword, &context) {
                completion_items.push(CompletionItem {
                    ...
                    filter_text: Some(keyword.keyword.to_string()), // Used for client-side filtering
                    sort_text: Some(format!("0_{}", keyword.keyword)), // Keywords get highest priority
                });
            }
        }
